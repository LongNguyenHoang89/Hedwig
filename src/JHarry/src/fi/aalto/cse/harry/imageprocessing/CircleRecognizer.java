	...
	public List<RectanleDimensions> recognize(BufferedImage imageFromClient) {
		...
		if (!webcam_image.empty()) {
			...
			/*Imgproc.HoughCircles(thresholded, circles,
					Imgproc.CV_HOUGH_GRADIENT, 2, thresholded.height() / 4,
					500, 50, 0, 0);*/

			/*

			int rows = circles.rows();
			int elemSize = (int) circles.elemSize(); // Returns 12 (3 *
														// 4bytes in a
														// float)
			float[] data2 = new float[rows * elemSize / 4];
			if (data2.length > 0) {
				circles.get(0, 0, data2); // Points to the first element
											// and reads the whole thing
				// into data2
				for (int i = 0; i < data2.length; i = i + 3) {
					int radius = (int) data2[i + 2];

					int x = ((int) data2[i] - radius);
				...
				if(data3 != null && data3.length > 0) {
					...
					for (int i = 0; i < data3.length; i = i + 4) {
						...
						System.out.println("X1: " + x1 + ", Y1: " + y1
								+ ", X2: " + x2 + ", Y2: " + y2);
						/*Core.line(webcam_image, new Point(x1, y1),
								new Point(x2, y2), new Scalar(100, 10,
										10), 3);*/
						int startX = (int) x1;
						//int startY = (int) y1;
						int startY = 0;
