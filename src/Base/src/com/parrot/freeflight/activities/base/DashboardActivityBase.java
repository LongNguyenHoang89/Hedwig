...
// There is no need to register this activity in the manifest as this is a base
// activity for others.
public class DashboardActivityBase extends ParrotActivity implements
	OnClickListener, MediaStorageReceiverDelegate {
    protected static final String TAG = "DashboardActivity";

    public enum EPhotoVideoState {
	UNKNOWN, READY, NO_MEDIA, NO_SDCARD
    }
    ...
    private CheckedTextView btnHedwigConfig;
	...
	// btnGetYourDrone = (CheckedTextView)
	// findViewById(R.id.btnGetYourDrone);
	btnHedwigConfig = (CheckedTextView) findViewById(R.id.btnHedwigConfig);
    }
	...
	btnHedwigConfig.setOnClickListener(this);
    }
	...
	btnPhotosVideos.setChecked(getPhotoVideoState().equals(
		EPhotoVideoState.READY));
	...
	case R.id.btnPhotosVideos:
		...
		showErrorMessageForTime(
			v,
			getString(R.string.there_is_no_flight_photos_or_videos_saved_in_your_phone),
			2000);
		break;
	    case NO_SDCARD:
		showErrorMessageForTime(v,
			getString(R.string.NO_SD_CARD_INSERTED), 2000);
		break;
	    default:
	...
	case R.id.btnFirmwareUpdate:
		...
		showErrorMessageForTime(v,
			getString(R.string.your_ar_drone_is_up_to_date), 2000);
	    }
	    break;
	case R.id.btnGames:
	    // Open Parrot Games
		...
		showErrorMessageForTime(v,
			getString(R.string.we_have_no_games_yet), 2000);
	    }
	    break;
	/*
	 * case R.id.btnGetYourDrone: // Open get your drone if
	 * (!isGuestSpaceEnabled() || !onStartGuestSpace()) {
	 * showErrorMessageForTime(v, getString(R.string.not_implemented_yet),
	 * 2000); } break;
	 */
	case R.id.btnHedwigConfig: {
	    onStartHedWig();
	    break;
	}
	}
	...
	if (buttonNok == null) {
	    buttonNok = (TextView) inflateView(R.layout.dashboard_button_nok,
		    parent, false);
    ...
    protected void showAlertDialog(String title, String message,
	    final Runnable actionOnDismiss) {
