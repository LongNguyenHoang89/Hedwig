...
public class DashboardActivity extends DashboardActivityBase implements
	...
	NetworkChangeReceiverDelegate, DroneFirmwareCheckReceiverDelegate,
	DroneConnectionChangeReceiverDelegate, MediaReadyDelegate {
	...
	broadcastManager.registerReceiver(droneFirmwareCheckReceiver,
		new IntentFilter(
			DroneControlService.DRONE_FIRMWARE_CHECK_ACTION));

	IntentFilter mediaReadyFilter = new IntentFilter();
	mediaReadyFilter
		.addAction(DroneControlService.NEW_MEDIA_IS_AVAILABLE_ACTION);
	mediaReadyFilter
		.addAction(TranscodingService.NEW_MEDIA_IS_AVAILABLE_ACTION);
	...
	HedwigLog.logFunction(this, "onResume");
	super.onResume();
	...
	if (firmwareUpdateAvailable) {
	    if (mService != null && mService.isUSBInserted()
		    && !FirmwareUpdateService.isRunning()) {
		onNotifyAboutUSBStickRemove();
	    } else {
		Intent updatefirmwareActivity = new Intent(this,
	...
	intent.putExtra(BrowserActivity.URL,
		getString(R.string.url_aa_register));
	startActivity(intent);
	...
	Log.d(TAG,
		"Network state has changed. State is: "
	...
	if (checkFirmwareTask != null
		&& checkFirmwareTask.getStatus() != Status.FINISHED) {
	...
	if (checkDroneConnectionTask != null
		&& checkDroneConnectionTask.getStatus() != Status.FINISHED) {
	...
	if (Build.VERSION.SDK_INT >= 11) {
	    checkDroneConnectionTask.executeOnExecutor(
		    CheckDroneNetworkAvailabilityTask.THREAD_POOL_EXECUTOR,
		    this);
	} else {
	...
	if (!taskRunning(checkAcademyAvailabilityTask)) {
	    ...
	    if (Build.VERSION.SDK_INT >= 11) {
		checkAcademyAvailabilityTask
			.executeOnExecutor(
				CheckAcademyAvailabilityTask.THREAD_POOL_EXECUTOR,
				this);
	    } else {
	...
	showAlertDialog(
		getString(R.string.Location_services_alert),
		getString(R.string.If_you_want_to_store_your_location_anc_access_your_media_enable_it),
		null);
    }
	...
	showAlertDialog(
		...
		getString(R.string.Please_unplug_your_USB_key_from_the_ARDrone_now),
		actionOnDismiss);
    }
