...
public abstract class JoystickBase extends Sprite {
	...
	if (thumbAbsoluteId != 0) {
	    thumbAbsolute = new GLSprite(context.getResources(),
		    thumbAbsoluteId);
	}
	...
	if (bg != null)
	    bg.onDraw(canvas, centerX - (bg.width >> 1) - margin.left,
		    inverseY(centerY - (bg.height >> 1)));
	...
	if (bg != null)
	    bg.onDraw(gl, centerX - (bg.width >> 1),
		    (centerY - (bg.height >> 1)));

	if (absolute) {
	    if (thumbAbsolute != null) {
		thumbAbsolute.onDraw(gl, thumbCenterX
			- (thumbNormal.width >> 1),
	...
	case MotionEvent.ACTION_DOWN:
	    ...
	    if (fingerId == -1
		    && activationRect.contains(event.getX(pointerIdx),
			    event.getY(pointerIdx))) {
	...
	return -1 * ((centerX - x) - (radius - (controlRatio * (radius * 2))))
		/ ((controlRatio * (radius * 2)));
    }

    private float getYValue(float centerY, float y, float radius) {
	return -1
		* ((centerY - inverseY(y)) - (radius - (controlRatio * (radius * 2))))
		/ ((controlRatio * (radius * 2)));
    }
